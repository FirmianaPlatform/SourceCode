Ext.define('gar.view.Notice', {
	...
	columns : [{
					...
					handler : function(grid, rowIndex, colIndex, record) {
						...
						if (rec.get('status') == 'done') {
														...
														handler : function() {
															...
															var tree = Ext.create('gar.view.CompareTree', {
																...
																onGroupDone : function(parentNode, childNode) {
																	var nodeStore = this.getRootNode();
																	columndata = oldcolumndata
																	// console.log(oldcolumndata)
																	newcolumndata = []
																	if (columndata[0].dataIndex == 'Sequence') {
																		for (i = 0; i < 4; i++) {
									...
									grid = Ext.create('gar.view.MBRun', {
												...
												dockedItems : [{
															...
															items : [{
																			...
																			var tree = Ext.create('gar.view.CompareTree', {
																						...
																						onGroupDone : function(parentNode, childNode) {
																							...
																							nodeStore.eachChild(function(n) {
																										if (n.isLeaf()) {
																										...
																										} else if (n.raw.text.indexOf('Dustbin') != -1) {
																											var k = new Object();
																											k["text"] = n.data.text
																											// console.log(n)
																											var b = []
																											for (i = 0; i < n.childNodes.length; i++) {
																												var x = new Object();
																												pp = n.childNodes[i].raw.text.split('_')
																												// x["checked"]
																												// =
																												// n.childNodes[i].data.checked
																												for (j = 0; j < columndata.length; j++) {
																													// console.log(columndata[j])
																													if (columndata[j].text.indexOf(pp[0]) != -1) {
																														x = columndata[j]
																													}
																												}
																												x["checked"] = n.childNodes[i].data.checked
																												b.push(x)
																											}
																											k["columns"] = b
																											newcolumndata.push(k)
																										}
																									})
																							columndata = newcolumndata
																							...
																							for (i = 0; i < columndata.length; i++) {
																								if (columndata[i].text == 'Annotation') {
																								} else if (String(columndata[i].text).indexOf("Group") != -1 || String(columndata[i].text).indexOf("OOT") != -1) {
																									temp = -1
																									pp = 0
																									for (j = 0; j < columndata[i].columns.length; j++) {
																										if (columndata[i].columns[j].checked) {
																											temp = j
																											pp = pp + 1
																											controlstring = columndata[i].columns[j].columns[0].columns[0].dataIndex
																										}
																									}
																									if (temp == -1 || pp > 1) {
																										Ext.Msg.alert("You should choose 1 Experiment as control")
																									} else {
																										for (j = 0; j < columndata[i].columns.length; j++) {
																											var examstring = String(columndata[i].columns[j].columns[0].columns[0].dataIndex)
																											columndata[i].columns[j].columns[0].columns[0].renderer = function(value, metaData, record, row, col, store, gridView) {
																												return value.toExponential(2)
																											}
																											if (j != temp) {
																												columndata[i].columns[j].columns[0].columns[2].filter = {
																													type : 'float',
																													encode : 'true'
																												}
																												columndata[i].columns[j].columns[0].columns[2].sortable = true
																												columndata[i].columns[j].columns[0].columns[2].dataIndex = columndata[i].columns[j].columns[0].columns[0].dataIndex + 'VS' + controlstring
																											} else {
																												columndata[i].columns[j].columns[0].columns[2].dataIndex = columndata[i].columns[j].columns[0].columns[0].dataIndex.split('VS')[0]
																												columndata[i].columns[j].columns[0].columns[2].renderer = function(value, record) {
																													return 1
																												}
																											}
																										}
																		...
																		handler : function() {
																			// temp_filter =
																			// Ext.getCmp(timestamp).filters.buildQuery(grid.filters.getFilterData());
																			// console.log(temp_filter[filter])
																			data = Ext.getCmp(timestamp).filters.getFilterData()
																			// store.getProxy().extraParams['download']
																			// =
																			// 'yes';
																		...
																		handler : function() {
																			...
																			// columns=''
																			// tempcol =
																			// Ext.getCmp(timestamp).columnManager.columns
																			// for (i = 0; i < tempcol.length; i++) {
																			// columns = columns + tempcol[i].text + ';'
																			// ddidx = ddidx + tempcol[i].dataIndex +
																			// ';'
							...
							Ext.Ajax.request({
										...
										success : function(response) {
											...
											for (i = 0; i < columndata.length; i++) {
												...
												if (String(columndata[i].text).indexOf("Medium") != -1 || String(columndata[i].text).indexOf("Var") != -1 || String(columndata[i].text).indexOf("Average") != -1) {
													columndata[i].renderer = function(value, metaData, record, row, col, store, gridView) {
														return value.toExponential(2);
													}
												}
							...
							if (rec.get('description') != '-')
								...
								desc = 'Comparison Search of ' + rec.get('exp_name')
							tab.add({
										title : desc,
										iconCls : 'tabs',
										closable : true,
				...
				renderer : function(value, metaData, record, rowIndex, colIndex, store) {
					cssstring = '<div title="Click to view Peptide List" class="x-grid3-cell-inner" style="text-align:center;';
					cssstring += '">' + "<a href='#'>"
					metaData.tdAttr = "data-qtip='" + value + "'";
