...
def get_obj_MS1(conn, meta, sid, gt, lt):
    ...
    mm = select([db.c.scan_num, db.c.rt, db.c.id]).where(
        and_(db.c.search_id == sid, db.c.rt < lt, db.c.rt > gt))
...
def get_mz_rt(conn, meta, ms2_id):
    ...
    mz, rt = conn.execution_options(autocommit=True).execute(
        select([db.c.pre_mz, db.c.rt]).where(db.c.id == ms2_id)).fetchone()
...
def calcAreas(sid, filename, dRT, dMZ, ins_pep):
    ...
    # Dir of .txt files of peaks
    dir = '%s/ms_peak/%s/' % (NFS89_dir, filename)
    # Dir of .txt files of peaks
    dir_MS1 = '%s/ms_peak/%s/MS1.txt' % (NFS89_dir, filename)
    ...
    if os.path.isfile(dir_MS1):
        ...
        for line in MS1_file:
            ...
            ms_no, mz_temp, intensity_temp = int(
                ms_no), float(mz_temp), float(intensity_temp)
            if ms_no != ms_no_old:
                i += 1
                # Record the order of every ms1 txt file appeared
                filedict[ms_no] = i
                if i != 0:
    ...
    ###############################################
        ...
        MS1_list = conn.execution_options(autocommit=True).execute(
            'select scan_num from gardener_ms1 where search_id=%s ORDER BY id' % sid)
    ...
    for pep_index in range(PepNum):
        ...
        pep_MS1 = []
        obj_MS1 = conn.execution_options(autocommit=True).execute(
            'select * from gardener_ms1 where search_id=%s and rt>=%s and rt<=%s ORDER BY scan_num' % (sid, gt, lt))
