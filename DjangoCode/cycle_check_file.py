...
def fileReady(conn, meta, eid):
    ...
    s = e.update().where(e.c.id == eid).values(
        {e.c.stage: 0, e.c.state: 'uploaded'})
    res = conn.execute(s)
    return res
...
def expPrepare(base, exp_info):
    ...
    exp_info['num_files'] = int(
        num_rep) * int(num_fra) if format != 'wiff' else int(num_rep) * int(num_fra) * 2
...
def get_instru_format_wf(conn, meta, eid):
    ...
    exp = Table('gardener_experiment', meta,
...
def getNoFileExp(conn, meta):
    ...
    exp = Table('gardener_experiment', meta,
                autoload=True, autoload_with=engine)
    sel = select([exp.c.id, exp.c.name, exp.c.priority]).where(and_(
        exp.c.stage == -1, exp.c.started == 0, exp.c.is_deleted == 0)).order_by(exp.c.id)
    res = conn.execution_options(autocommit=True).execute(sel)
    if res != None:
        for row in res:
            exp_list.append(
                (row[exp.c.id], row[exp.c.name], row[exp.c.priority]))
    # print 'Undone exp list:',','.join(exp_list),'\n'
    if len(exp_list) != 0:
        exp_list.sort(key=lambda x: x[2], reverse=1)
        exp_line = [e[1] for e in exp_list]
        tmp = '%s\nTime : %s\nUndone exp : %s' % (
            dividline, date_now(), ','.join(exp_line))
        logPrint(tmp)
...
def checkFilePath(exp_info):
    def check_file_NAS(exp_info):
        ...
        if total == all_file:
            return 'nas'
        # print 'Not enough files in NAS
        # path:%s(%s/%s)'%(path_bprc,total,num_fra)
        else:
            logPrint('Error:Not enough files in outer path:[%s](%s/%s)\n%s' % (
                exp_info['path_bprc'], total, all_file, SPLIT_LINE))
            return 'err'
...
def copy_from_bprc(path_bprc, path, format):
    ...
    logPrint('end   :%s...Complete copying files from %s...' %
             (date_now(), path_bprc))
    return 1
...
def __main__():
    ...
    # '/usr/local/galaxyDATA01/bprc_dingchen'
    base_bprc = ConfigSectionMap("Galaxy")['bprc_nas']
    # '/usr/local/galaxyDATA01/data/ftpdata'
    base_ftp_tmp = ConfigSectionMap("Galaxy")['ftp']
