...
def __main__():
    ...
    for e_num in e_num_list:
        ...
        t = 'delete from gardener_search where exp_id=%s'%exp_id
        conn.execution_options(autocommit=True).execute(t)
        t = 'UPDATE gardener_experiment set stage=0, started=0, num_spectrum=0, num_peptide=0, num_isoform=0, num_gene=0 where id=%s' %exp_id
        conn.execute(t)
