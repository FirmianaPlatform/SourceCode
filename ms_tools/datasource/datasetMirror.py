...
def expPrepare(base, exp_info):
    ...
    exp_info['source_lib_folder_name'] = 'API imported %s' %exp_info['source_ExpName']
    exp_info['target_lib_folder_name'] = 'API imported %s' %exp_info['target_ExpName']
...
def getExp(eName):
    ...
    sel = select([exp.c.id, exp.c.name, exp.c.priority,exp.c.instrument,exp.c.species,exp.c.num_fraction,exp.c.num_repeat]).where(exp.c.name==eName)
...
def renameFile(path, exp_info):
    ...
    for file in files: 
        ...
        if len(ext_parts) > 2:
            newname = ext_parts[0]+'_E'+str(exp_Num)+'_F'+str(num_f)+'_R'+str(num_r)+'.'+ext_parts[-2]+'.'+ext_parts[-1]
        else:
            newname = ext_parts[0]+'_E'+str(exp_Num)+'_F'+str(num_f)+'_R'+str(num_r)+'.'+format
...
def uploadLibData(exp_info, library_id, lib_folder_id, path):
    ...
    api_key = exp_info['api_key']
    api_url = exp_info['api_url']
    lib_folder_name = exp_info['target_lib_folder_name']
    #dict_file = {}
    #i = 0
    #libset = display(api_key, api_url + "libraries/%s/contents" % library_id, return_formatted = False)
    #for lib in libset:
...
def __main__():
    ...
    exp_info['e_name']   = exp_info['target_ExpName'] #'Exp001608' #experiment[1]
    ...
    #===================================================================
    target_Exp_folder,x1,x2 = getFilePath(base, base_bprc, base_ftp_tmp, exp_info)
    ...
    if lib_exist == 1:
        ...
        for fname,v in dict_file.iteritems():
            ...
            res = display(exp_info['api_key'], exp_info['api_url'] + "libraries/%s/contents/%s" % (library_id,content_id), return_formatted = False)
            fPath = res['file_name']
            oldExpStr = '_E'+ exp_info['source_ExpName'][3:] + '_F'
            newExpStr = '_E'+ exp_info['target_ExpName'][3:] + '_F'
            targetLink = os.path.join( target_Exp_folder, fname.replace( oldExpStr,newExpStr ) )
            #print fPath
            #print targetLink
