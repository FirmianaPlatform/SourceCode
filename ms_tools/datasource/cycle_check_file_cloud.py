...
def getNoFileExp(conn,meta):
    ...
    where(and_(exp.c.stage==-1,exp.c.started==0,exp.c.is_deleted==0,exp.c.file_source=='aliyun')).order_by(exp.c.id)
...
def getFilePath(base, base_bprc, base_ftp_tmp, exp_info):
    ...
    exp_info['path'] = os.path.join(base, email, instru_folder, exp_Num)
