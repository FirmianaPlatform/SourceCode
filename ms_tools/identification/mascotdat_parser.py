...
def calcAreas(file_data, options, dRT, dMZ, ins_pep):
    ...
    dir_peakFile = '%s/database/files/ms_peak/%s/' % (GALAXY_ROOT, filename)  # Dir of .txt files of peaks
    dir_MS1_tmp = '%s/database/files/ms_peak_tmp/%s/MS1.txt' % (GALAXY_ROOT, filename)  # Dir of .txt files of peaks
    dir_MS1 = '%s/database/files/ms_peak/%s/MS1.txt' % (GALAXY_ROOT, filename)  # Dir of .txt files of peaks
    if not os.path.isfile(dir_MS1):
        if os.path.isfile(dir_MS1_tmp): 
            shutil.move(dir_MS1_tmp, dir_MS1)
    ...
    ###############################################
        ...
        for row in MS1_list :
            ...
            i += 1
            f = open(dir_peakFile + peak_file , 'r')
            for line in f:
                (ms_no, mz_temp, intensity_temp) = line.split(',')
...
def startProteinAssemble(eid, peptide_list, pep_list_decoy, fdrThres, ionThres):
    ...
    '''
    Now start test compare with PD
    '''
    qvalueThres = 1
    ...
    while 1:
        ...
        # If FDR is good
        #=======================================================================
        print '# # # # # Protein FDR:', FDR
        print '# # # # # Peptide FDR:', FDR_pep
        print 'protein number =', target_num
        print 'peptide number =', total_pep_1
        if FDR < fdrThres:
            if times == 1: break
            FDR_good = FDR
...
def getProGeneFromDB(dict_uni_pro, group_result, sid, eid, fdr, dict_pro_length):
    ...
    for row in group_result:
        ...
        if int(taxid) == 8355:#xenopus, Lei Song
            ...
            gid = -1
        elif int(taxid) == 7955:#zebrafish, Bowen Zhong
            gid = -1
        elif int(taxid) == 4538:#african rice, Bowen Zhong
            gid = -1
        else:
            ''' Get Gene Info '''
        ...
        if gid == -1 :
            ...
            '''    
            #eid gardener_experiment
            if 'Schizosaccharomyces pombe' == species:
                (symbol, desc) = get_yeast_gene_from_db(acc_short)
            if eid in range(2211,2227) or eid == 2167 or eid in range(2351,2376):
                (symbol, desc) = get_xeno_gene_from_humandb(acc_short)
            if eid in range(4056, 4058) or eid == 4322 or 'Danio rerio (Zebra fish)' == species:
                (symbol, desc) = get_zebrafish_gene_from_db(acc_short)
            if eid in range(4110, 4114) or "Oryza glaberrima (African rice)" == species:
                (symbol, desc) = get_african_rice_gene_from_db(acc_short)
            #elif int(taxid) == 8355:
...
def mergePercolatorFile(tab_file, fea_file, type):
    ...
    for line in f_tab:
        ...
        line = line.split('\t')
        query_rank = line[0]
        # Sometimes q value failed,because tab file was written twicely 
        qvalue = float(line[2])
...
def parseMascotDat(options, file_data):
    ...
    msparser_min_ionscore = 0#
    ionThres = 0#options.min_ion
    ...
    MSRES1 = msparser.ms_mascotresults.MSRES_MUDPIT_PROTEIN_SCORE
    MSRES2 = msparser.ms_mascotresults.MSRES_GROUP_PROTEINS
    MSRES3 = msparser.ms_mascotresults.MSRES_MAXHITS_OVERRIDES_MINPROB
    MSRES4 = msparser.ms_mascotresults.MSRES_DECOY
    
    msresFlags       = MSRES1 | MSRES2 | MSRES3
    msresFlags_decoy = MSRES1 | MSRES2 | MSRES3 | MSRES4
...
def storeDB(conn, meta, file_data, options):
    ...
    if max_repeat == 1:
        ...
        updateExpStage(conn, meta, file_data)
#         send_complete_mail('hzqnq@163.com','Exp'+file_data['exp_id']+' done','Congratulations,workflow completed in %s hours!'%total_time)
